...
class Error:
    ...
    def SliceX(self, x):
        ...
        for i in range(res):
            for j in range(res):
                vector, vector_original = Error.sample_fields(self, np.array([x[i], y[j], x]))
                u[i][j] = vector[0]
                v[i][j] = vector[1]
                w[i][j] = vector[2]
        ...
        Error.ShowQuiver_original(self, x, y, v_orig, w_orig, x, 'x')
    ...
    def SliceY(self, y):
        ...
        for i in range(res):
            for j in range(res):
                vector, vector_original = Error.sample_fields(self, np.array([x[i], y[j], y]))
                u[i][j] = vector[0]
                v[i][j] = vector[1]
                w[i][j] = vector[2]
        ...
        Error.ShowError(self, x, y, error, y, 'y')
        Error.ShowQuiver(self, x, y, u, w, y, 'y')
        Error.ShowQuiver_original(self, x, y, u_orig, w_orig, y, 'y')
    ...
    def ShowQuiver_original(self, x, y, u, v, height, axis):
        ...
        plt.figure(figsize=(15, 15))
        plt.quiver(X, Y, v, u)
        plt.title('Original Fieldat' + axis + '= ' + str(height))
        plt.gca().set_aspect('equal', adjustable='box')
        plt.show()
